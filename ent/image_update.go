...
func (iu *ImageUpdate) SetStoragePath(s string) *ImageUpdate {
	iu.mutation.SetStoragePath(s)
	return iu
}
...
func (iu *ImageUpdate) check() error {
	if v, ok := iu.mutation.StoragePath(); ok {
		if err := image.StoragePathValidator(v); err != nil {
...
func (iu *ImageUpdate) sqlSave(ctx context.Context) (n int, err error) {
	...
	if value, ok := iu.mutation.StoragePath(); ok {
		_spec.Fields.Set = append(_spec.Fields.Set, &sqlgraph.FieldSpec{
			Type:   field.TypeString,
			Value:  value,
			Column: image.FieldStoragePath,
		})
	}
...
// SetStoragePath sets the "storage_path" field.
func (iuo *ImageUpdateOne) SetStoragePath(s string) *ImageUpdateOne {
	iuo.mutation.SetStoragePath(s)
	return iuo
}
...
func (iuo *ImageUpdateOne) check() error {
	if v, ok := iuo.mutation.StoragePath(); ok {
		if err := image.StoragePathValidator(v); err != nil {
			return &ValidationError{Name: "storage_path", err: fmt.Errorf("ent: validator failed for field \"storage_path\": %w", err)}
		}
	}
...
func (iuo *ImageUpdateOne) sqlSave(ctx context.Context) (_node *Image, err error) {
	...
	if value, ok := iuo.mutation.StoragePath(); ok {
		_spec.Fields.Set = append(_spec.Fields.Set, &sqlgraph.FieldSpec{
			Type:   field.TypeString,
			Value:  value,
			Column: image.FieldStoragePath,
