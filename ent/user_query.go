...
func (uq *UserQuery) QueryAudio() *AudioClipQuery {
	query := &AudioClipQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) WithAudio(opts ...func(*AudioClipQuery)) *UserQuery {
	query := &AudioClipQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withImages; query != nil {
		...
		for _, n := range neighbors {
			fk := n.user_images
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_images" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_images" returned %v for node %v`, *fk, n.ID)
			}
	...
	if query := uq.withAudio; query != nil {
		...
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.Audio = []*AudioClip{}
		}
		query.withFKs = true
		query.Where(predicate.AudioClip(func(s *sql.Selector) {
			s.Where(sql.InValues(user.AudioColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.user_audio
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_audio" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_audio" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Audio = append(node.Edges.Audio, n)
		}
	}
	...
	if query := uq.withRole; query != nil {
		...
		query.Where(predicate.Role(func(s *sql.Selector) {
			s.Where(sql.InValues(user.RoleColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
