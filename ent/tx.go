...
func (tx *Tx) init() {
	tx.AudioClip = NewAudioClipClient(tx.config)
	tx.DiscordUser = NewDiscordUserClient(tx.config)
	tx.Image = NewImageClient(tx.config)
	tx.Role = NewRoleClient(tx.config)
	tx.User = NewUserClient(tx.config)
	tx.VRCUser = NewVRCUserClient(tx.config)
}
...
// applies a query, for example: AudioClip.QueryXXX(), the query will be executed
